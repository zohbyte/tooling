...
type (
	...
	Event struct {
		...
		DiscordWebhooks     *[]string                 `json:",omitempty"`
	...
	discordFlagWebhookUrls       []string
	discordFlagWebhookTemplate   DiscordFlagWebhookTemplate
	...
	// DiscordFlagWebhookTemplate holds the customisable message format for
	// Discord flag-alert webhooks. Placeholders: {tag}, {count}, {time}.
	DiscordFlagWebhookTemplate struct {
		Title          string
		Description    string
		Color          string   // Hex colour string, e.g. "#FF0000"
		MentionUserIDs []string // Discord user IDs to ping, e.g. ["123456789012345678"]
	}
...
nextStateFile:
	...
	mgr.discordFlagWebhookUrls = s.DiscordFlagWebhookUrls
	mgr.discordFlagWebhookTemplate = defaultDiscordFlagWebhookTemplate()
	if s.DiscordFlagWebhookTemplate.Title != "" {
		mgr.discordFlagWebhookTemplate.Title = s.DiscordFlagWebhookTemplate.Title
	}
	if s.DiscordFlagWebhookTemplate.Description != "" {
		mgr.discordFlagWebhookTemplate.Description = s.DiscordFlagWebhookTemplate.Description
	}
	if s.DiscordFlagWebhookTemplate.Color != "" {
		mgr.discordFlagWebhookTemplate.Color = s.DiscordFlagWebhookTemplate.Color
	}
	if s.DiscordFlagWebhookTemplate.MentionUserIDs != nil {
		mgr.discordFlagWebhookTemplate.MentionUserIDs = s.DiscordFlagWebhookTemplate.MentionUserIDs
	}
...
func (mgr *Manager) saveState() error {
	j := stateFile{
		...
		DiscordFlagWebhookUrls:     mgr.discordFlagWebhookUrls,
		DiscordFlagWebhookTemplate: mgr.discordFlagWebhookTemplate,
...
func (mgr *Manager) updateTagJob(name string, t tag, tagDetails map[string]query.TagDetails, converters map[string]index.ConverterAccess, indexes []*index.Reader, releaser indexReleaser) {
	// Capture pre-evaluation certain match count for Discord flag webhook trigger.
	// "Certain" = matches that are NOT in the uncertain set.
	originalCertain := t.Matches.Copy()
	originalCertain.Sub(t.Uncertain)
	originalCertainCount := int(originalCertain.OnesCount())
	...
	newMatchCount := int(t.Matches.OnesCount())
	...
	mgr.jobs <- func() {
		...
		if ot, ok := mgr.tags[name]; ok && ot.definition == t.definition {
			...
			// Trigger Discord flag webhooks if this is a flag-related tag with new matches.
			// newlyMatchedCount = streams that were uncertain (newly added from latest PCAP)
			// and are now confirmed to match this tag.
			newlyMatchedCount := newMatchCount - originalCertainCount
			if len(mgr.discordFlagWebhookUrls) > 0 && newlyMatchedCount > 0 && strings.Contains(strings.ToLower(name), "flag") {
				webhookUrls := append([]string(nil), mgr.discordFlagWebhookUrls...)
				go mgr.triggerDiscordFlagWebhooks(name, newlyMatchedCount, webhookUrls)
			}
		}
...
func (mgr *Manager) ListDiscordFlagWebhooks() []string {
	c := make(chan []string)
	mgr.jobs <- func() {
		if mgr.discordFlagWebhookUrls == nil {
			c <- []string{}
		} else {
			c <- mgr.discordFlagWebhookUrls
		}
		close(c)
	}
	return <-c
...
func (mgr *Manager) AddDiscordFlagWebhook(url string) error {
	c := make(chan error)
	mgr.jobs <- func() {
		for _, u := range mgr.discordFlagWebhookUrls {
			if u == url {
				c <- fmt.Errorf("error: url %q already exists", url)
				close(c)
				return
		...
		mgr.discordFlagWebhookUrls = append(mgr.discordFlagWebhookUrls, url)
		mgr.event(Event{
			Type:            "discordWebhooksUpdated",
			DiscordWebhooks: &mgr.discordFlagWebhookUrls,
		})
		c <- mgr.saveState()
		close(c)
	}
...
func (mgr *Manager) DelDiscordFlagWebhook(url string) error {
	c := make(chan error)
	mgr.jobs <- func() {
		for i, u := range mgr.discordFlagWebhookUrls {
			if u == url {
				mgr.discordFlagWebhookUrls = append(mgr.discordFlagWebhookUrls[:i], mgr.discordFlagWebhookUrls[i+1:]...)
				mgr.event(Event{
					Type:            "discordWebhooksUpdated",
					DiscordWebhooks: &mgr.discordFlagWebhookUrls,
				})
...
func (mgr *Manager) GetDiscordFlagWebhookTemplate() DiscordFlagWebhookTemplate {
	c := make(chan DiscordFlagWebhookTemplate, 1)
	mgr.jobs <- func() {
		c <- mgr.discordFlagWebhookTemplate
	}
	return <-c
}

func (mgr *Manager) SetDiscordFlagWebhookTemplate(tmpl DiscordFlagWebhookTemplate) error {
	c := make(chan error, 1)
	mgr.jobs <- func() {
		mgr.discordFlagWebhookTemplate = tmpl
		c <- mgr.saveState()
	}
	return <-c
}

// defaultDiscordFlagWebhookTemplate returns the built-in defaults for the
// Discord flag-alert message.  Placeholders: {tag}, {count}, {time}.
func defaultDiscordFlagWebhookTemplate() DiscordFlagWebhookTemplate {
	return DiscordFlagWebhookTemplate{
		Title:          "⚠️ Flag Alert: Possible Flag Exfiltration Detected",
		Description:    "Tag **`{tag}`** matched **{count}** new stream(s) in the latest PCAP analysis at {time}. Flags may be going out!",
		Color:          "#FF0000",
		MentionUserIDs: []string{},
	}
}

// hexColorToDiscordInt converts a CSS-style hex colour string (e.g. "#FF0000"
// or "FF0000") to the integer that Discord expects in embed payloads.
func hexColorToDiscordInt(hex string) int {
	hex = strings.TrimPrefix(hex, "#")
	if len(hex) != 6 {
		return 16711680 // fallback: red
	}
...
// applyDiscordTemplate substitutes {tag}, {count}, and {time} placeholders in s.
func applyDiscordTemplate(s, tagName string, matchCount int) string {
	s = strings.ReplaceAll(s, "{tag}", tagName)
	s = strings.ReplaceAll(s, "{count}", strconv.Itoa(matchCount))
	s = strings.ReplaceAll(s, "{time}", time.Now().UTC().Format("2006-01-02 15:04:05 UTC"))
	return s
}

func (mgr *Manager) triggerDiscordFlagWebhooks(tagName string, matchCount int, webhookUrls []string) {
	tmpl := mgr.discordFlagWebhookTemplate
	...
	// Build @mention string for the content field (pings appear above the embed).
	content := ""
	if len(tmpl.MentionUserIDs) > 0 {
		parts := make([]string, len(tmpl.MentionUserIDs))
		for i, id := range tmpl.MentionUserIDs {
			parts[i] = "<@" + id + ">"
		}
	...
	payload := discordPayload{
		...
		Embeds: []discordEmbed{
			{
				Title:       applyDiscordTemplate(tmpl.Title, tagName, matchCount),
				Description: applyDiscordTemplate(tmpl.Description, tagName, matchCount),
				Color:       hexColorToDiscordInt(tmpl.Color),
	...
	if err != nil {
		log.Printf("error: discord flag webhook body json encode failed: %v\n", err)
		return
	}
	for _, webhookUrl := range webhookUrls {
		go func(u string) {
			err := func() error {
				...
				if err != nil {
					return fmt.Errorf("failed to create discord flag webhook request: %w", err)
				}
				req.Header.Set("Content-Type", "application/json")
				res, err := http.DefaultClient.Do(req)
				if err != nil {
					return fmt.Errorf("failed to send discord flag webhook request: %w", err)
				}
				defer res.Body.Close()
				// Discord webhooks return 204 No Content on success
				if res.StatusCode != 200 && res.StatusCode != 204 {
					return fmt.Errorf("discord flag webhook request failed: %q", res.Status)
				}
				return nil
			}()
			if err != nil {
				log.Printf("discord flag webhook error: %v\n", err)
			}
		}(webhookUrl)
	}
}
