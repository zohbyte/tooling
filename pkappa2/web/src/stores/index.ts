...
interface State {
  ...
  discordWebhooks: DiscordWebhooks | null;
  discordWebhookTemplate: DiscordWebhookTemplate | null;
...
export const useRootStore = defineStore("root", {
  ...
  actions: {
    ...
    async updateDiscordWebhooks() {
      return APIClient.getDiscordWebhooks()
        .then((data) => (this.discordWebhooks = data))
        .catch(handleAxiosDefaultError);
    },
    async addDiscordWebhook(url: string) {
      return APIClient.addDiscordWebhook(url)
        .then(() => this.updateDiscordWebhooks())
        .catch(handleAxiosDefaultError);
    },
    async delDiscordWebhook(url: string) {
      return APIClient.delDiscordWebhook(url)
        .then(() => this.updateDiscordWebhooks())
        .catch(handleAxiosDefaultError);
    },
    async fetchDiscordWebhookTemplate() {
      return APIClient.getDiscordWebhookTemplate()
        .then((data) => (this.discordWebhookTemplate = data))
        .catch(handleAxiosDefaultError);
    },
    async saveDiscordWebhookTemplate(tmpl: DiscordWebhookTemplate) {
      return APIClient.setDiscordWebhookTemplate(tmpl)
        .then(() => this.fetchDiscordWebhookTemplate())
        .catch(handleAxiosDefaultError);
    },
