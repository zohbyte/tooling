...
def _check_api_auth() -> bool:
    expected_user = os.getenv("TULIP_AUTH_USERNAME")
    expected_pw = os.getenv("TULIP_AUTH_PASSWORD")
    if not expected_user or not expected_pw:
        # No auth configured, open API
        return True
...
def _api_auth_guard():
    # only protect /api endpoints. frontend static is open.
...
def api_ping():
    ...
    return return_json_response({"status": "ok", "code": code})


@application.route("/api/info")
def api_info():
    return return_json_response({"name": "tulip", "version": "0.1.0", "description": "Tulip API adapter"})


@application.route("/api/teams")
def api_teams():
    # Reuse service list as team-like objects for scoring.
    return return_json_response([{"id": i + 1, "name": s.get("name", f"team_{i+1}"), "ip": s.get("ip"), "port": s.get("port")} for i, s in enumerate(services)])


@application.route("/api/scores")
def api_scores():
    # Return statistics from the existing Tulip stats engine.
    stats = []
    with db.connection() as c:
        outcomes = c.stats_query(database.StatsQuery())
        for service_name, score in outcomes.items():
            stats.append({"service": service_name, "score": score})
    return return_json_response(stats)


@application.route("/api/checks")
def api_checks():
    # Expose simple health checks
    return return_json_response({"uptime": True, "db": True, "ingestion": True})


@application.route("/api/flagids")
def api_flagids():
    # Basic flagid discovery endpoint (catagorized by service). In Tulip, tags are used.
    with db.connection() as c:
        tags = c.tag_list()
    return return_json_response(tags)


@application.route("/api/flagids/<int:service_id>")
def api_flagids_service(service_id):
    # Return metadata or placeholder for service-specific flagids
    return return_json_response({"service_id": service_id, "flagids": []})


@application.route("/api/flagids/<int:service_id>/<int:team_id>")
def api_flagids_service_team(service_id, team_id):
    # Return flagid details for a team and service.
    return return_json_response({"service_id": service_id, "team_id": team_id, "flagids": []})


@application.route("/api/submit", methods=["POST"])
def api_submit_flag():
    data = request.get_json(force=True)
    if not data or "flags" not in data or not isinstance(data["flags"], list):
        return return_json_response({"error": "flags field is required and must be a list"}, status=422)
    # simple capture; can be wired into game scoring logic
    with open("/tmp/flag_submissions.log", "a") as f:
        f.write(str(data) + "\n")
    return return_json_response({"result": "accepted", "submitted": len(data["flags"])})


@application.route("/api/rename", methods=["POST"])
def api_rename_team():
    data = request.get_json(force=True)
    if not data or "name" not in data:
        return return_json_response({"error": "name field is required"}, status=422)
    # No persisted team record in this prototype; just echo
    return return_json_response({"result": "renamed", "name": data["name"]})
